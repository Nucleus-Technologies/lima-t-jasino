    ...
    @include('customer.layouts.partials._banner', [
        "banner_content" => "<h2>" .$title. "</h2>
            <div class='page_link'>
                ...
                <a href=" .route('order.show', $order). ">" .$title. "</a>
            </div>
        "
    ])
...
@section('content')
    {{-- Order Details Area --}}
	<section class="order_details checkout_confirmation mb-5">
        <div class="container">
                ...
                <div class="col-lg-4">
                    <div class="details_item">
                        ...
                        <ul class="list">
                            ...
                            <li>
                                <span class="first">Date:</span> <strong>{{ format_date($order->payment->created_at) }}</strong>
                            </li>
                            <li>
                                <span class="first">Total:</span> <strong>@convert(total_o($order->id))</strong>
                            </li>
                            <li>
                                <span class="first">Payment method:</span> <strong>{{ format_payment_mode($order->payment->payment_mode) }}</strong>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            @isset($order->payment)
                <div class="w-100 bg-success text-uppercase payment-state">
                    Payment State : <i class="fas fa-check-circle ml-3 mr-1"></i> Paid
                </div>
            @else
                <div class="w-100 bg-danger text-uppercase payment-state">
                    Payment State : <i class="fas fa-times-circle ml-3 mr-1"></i> Not Paid
                </div>
            @endisset
        </div>
    </section>
